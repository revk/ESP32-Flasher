...
Note that `"build"` in the `"flash"` file expects an ESP/IDF 256 byte *app description* block at the specified address. The *app name* and *version* are strings from this, but the *build* time is taken from *date* and *time* fields and formatted as an ISO time, e.g. `2025-10-03T12:38:06`. As such the `ID:` value for this should be the same ISO date format (note, no `Z` or timezone suffix).
